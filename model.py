...
class MessageEmbedder(nn.Module):
    def __init__(
        ...
        self._tokenizer: AutoTokenizer = AutoTokenizer.from_pretrained(base_model)
        ...
        new_tokens = ['<video>', '</video>', '<link/>', '<attachment/>', '</user>'] + [f"<user{i}>" for i in range(self._context_length)]
    ...
    def forward(self, x, attn_mask, *args, **kwargs):
        last_hidden_state = self.underlying_model(x, attn_mask, *args, **kwargs).last_hidden_state
    ...
    def get_param_classes(self) -> dict[str, Any]:
        input_embeddings = self._underlying_model.get_input_embeddings()
        old_params = [p for n, p in self._underlying_model.named_parameters() if 'embeddings' not in n and 'attention_query' not in n] + [input_embeddings[:self._num_new_tokens]]
        new_params = [input_embeddings.weight[:-self._num_new_tokens]]
        if self._pooling_mode == 'attention':
